...
function applyApplicationSnapshot(
  ...
  if (
    runKey &&
    runKey !== lastReportedRunKey
  ) {
    lastReportedRunKey = runKey;
    activities.push(
      withActivity({
        label: 'Sync completed',
        detail: `${lastRun?.processed ?? 0} tracks processed in ${lastRun?.elapsedMs ?? 0} ms.`,
        status: 'success',
      }),
    );
  }
...
async function sendMessage(
  ...
  try {
    ...
    addAssistantMessage(
      extractResponse(result.result),
    );
    setCopilot('completed', null);
    apply({
      ...snapshot,
      busy: false,
      activities: [
        ...snapshot.activities,
        withActivity({
          label: 'Copilot responded',
          detail: 'Conversation completed successfully.',
          status: 'success',
        }),
      ].slice(-100),
    });
...
async function runSyncStart(): Promise<void> {
  ...
  apply({
    ...snapshot,
    sync: 'starting',
    syncDetail: 'Starting synchronization…',
  });
...
async function runSyncStop(): Promise<void> {
  ...
  apply({
    ...snapshot,
    sync: 'stopping',
    syncDetail: 'Stopping synchronization…',
  });
    ...
    apply({
      ...
      activities: [
        ...
        withActivity({
          ...
          detail: result.status ?? undefined,
          status: 'success',
        }),
      ].slice(-100),
    });
...
const ui =
  mountProductionUi({
    ...
    callbacks: {
      ...
      onStartSync: runSyncStart,
      onStopSync: runSyncStop,
      onRefresh: refresh,
    },
  });
  ...
  try {
    ...
    apply({
      ...
      activities: [
        ...snapshot.activities,
        withActivity({
          label: 'Copilot is not configured',
          detail: 'Set COPILOT_PROVIDER, COPILOT_API_KEY, and COPILOT_MODEL in the Electron environment.',
          status: 'warning',
        }),
      ].slice(-100),
    });
    ...
    apply({
      ...
      activities: [
        ...snapshot.activities,
        withActivity({
          label: 'Copilot status unavailable',
          detail: errorMessage(error),
          status: 'warning',
        }),
      ].slice(-100),
